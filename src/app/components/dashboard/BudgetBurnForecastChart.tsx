// src/app/components/dashboard/BudgetBurnForecastChart.tsx
import React from "react";
import { AreaChart, Area, XAxis, YAxis, Tooltip, ResponsiveContainer, ReferenceLine } from "recharts";
import { MOCK_BUDGET_BURN_DATA } from "./dashboardData";
import { TrendingUp, AlertCircle } from "lucide-react";

export function BudgetBurnForecastChart() {
  return (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-neutral-100 dark:border-neutral-800">
        <div>
          <h3 className="font-bold text-base text-neutral-900 dark:text-white flex items-center gap-2">
            ...
            Budget Burn Rate & Forecast Trajectory
          </h3>
          <p className="text-xs text-neutral-500 mt-0.5">
            Cumulative spend trajectory vs $10,000 monthly ceiling & projected pace.
          </p>
        </div>
        ...
        <div className="flex items-center gap-4 text-xs font-medium">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-neutral-600 dark:text-neutral-400">Actual Spend ($8.05K)</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-full bg-purple-400 border border-dashed" />
            <span className="text-neutral-600 dark:text-neutral-400">Pace Forecast ($8.69K)</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={MOCK_BUDGET_BURN_DATA} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            <defs>
              ...
              <linearGradient id="forecastSpendGrad" x1="0" y1="0" x2="0" y2="1">
            ...
            <Area type="monotone" dataKey="forecastSpend" stroke="#a855f7" strokeWidth={2} strokeDasharray="4 4" fill="url(#forecastSpendGrad)" name="Forecasted Spend" />
            <Area type="monotone" dataKey="actualSpend" stroke="#4f46e5" strokeWidth={3} fill="url(#actualSpendGrad)" name="Actual Spend" />
          </AreaChart>
        </ResponsiveContainer>
      </div>

      <div className="flex items-center gap-2 p-3 rounded-xl bg-blue-50/60 dark:bg-blue-950/30 border border-blue-200/60 dark:border-blue-900/50 text-xs text-blue-700 dark:text-blue-300">
        <AlertCircle className="w-4 h-4 shrink-0 text-blue-600" />
        <span>
          <strong>Budget Burn Insight:</strong> Based on current run-rate of $268.33/day, projected end-of-month spend is <strong>$8,694.00</strong> (86.9% of ceiling). No budget cap breach expected.
        </span>
      </div>
    </div>
  );
