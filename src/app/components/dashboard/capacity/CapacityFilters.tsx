// src/app/components/dashboard/capacity/CapacityFilters.tsx
...
export function CapacityFilters({ filters, onChangeFilter, onResetFilters }: CapacityFiltersProps) {
  return (
    <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-2xl p-4 shadow-xs space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3 text-xs">
        <div>
          <label className="text-[11px] font-semibold text-neutral-500 block mb-1">Limit Type</label>
          <select
            value={filters.selectedLimitType}
            onChange={(e) => onChangeFilter("selectedLimitType", e.target.value as FilterState["selectedLimitType"])}
          ...
          >
            <option value="all">All Limits (RPM & TPM)</option>
            <option value="rpm">RPM Only (Requests/Min)</option>
            <option value="tpm">TPM Only (Tokens/Min)</option>
          </select>
        </div>
